...
# print(topo_data)
np.savetxt(f"unitcell_avg_ldos_data_sem_yedge_2pi6_{mu}_eta_{eta}_cdw{cdw}.dat", topo_data, fmt="%12.6f")
# %%
data = np.loadtxt(f'unitcell_avg_ldos_data_sem_yedge_2pi6_{mu}_eta_{eta}_cdw{cdw}.dat')
for i in range(1, int(nx * ny / 2) + 1, 6*24):
    plt.plot(data[:, 0], data[:, i], linewidth=2)
# plt.axvline(x=0, color='black', linewidth=0.8)
# plt.axvline(x=-70, color='red', linewidth=0.8)
plt.hlines(0,-80,50, color='black', lw=0.8,ls='dotted')
# plt.ylim(0, 1.9)
# plt.xlim(-3,3)
...
# plt.grid()
plt.savefig(f"{path}xe_dos_OP_sem_yedge{mu}.png",dpi = 300)
plt.show()

dos_data = np.array([[data[:, 0][i], data[:, 1][i], data[:, int(nx * ny / 2)][i]] for i in range(len(data[:, 0]))])
np.savetxt(f'{path}energy_edge_bulk_DOS.dat',dos_data,fmt="%12.6f")
